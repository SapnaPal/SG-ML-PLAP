...
for filename in result/$1/$1_pdbqt/*.pdbqt; do
    echo "${filename%.*}"
    ./vina_package/build/linux/release/vina --receptor vina_receptor.pdbqt --ligand $filename --score_only --log ${filename%.*}.txt   #make sure the vina used from the vina_package
...
python ecif_vina_crystal_predcition.py -f result/$1/$1_vina_ecif_features.csv #update currect path
mv ECIF_VINA_crystal_trained_gbt_prediction.csv $1_ECIF_VINA_crystal_trained_gbt_prediction.csv
mv $1_ECIF_VINA_crystal_trained_gbt_prediction.csv result/$1/$1_ECIF_VINA_crystal_trained_gbt_prediction.csv #path of result directory
